...
Chassis::Chassis()
{
  ...
  // Setup Variables
  node_priv.getParam("odom_child_frame_id",odom_child_frame_id);
  node_priv.getParam("odom_header_frame_id",odom_header_frame_id);
  ...
  odom_pub  = node_priv.advertise<nav_msgs::Odometry>("wheel_odom", 10);
  // Setup Odometry
  x = y = theta = 0;
  for (int i = 0; i < 2; i++)
  {
