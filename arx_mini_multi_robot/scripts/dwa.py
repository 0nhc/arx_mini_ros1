...
# DWA基本参数
obs_range = float(rospy.get_param('~obsRange', 1.2))       # 障碍物感知范围
min_lidar_point = float(rospy.get_param('~minPoint', 3))    # 滤除小于此数目雷达点的障碍物
safe_range = float(rospy.get_param('~safeRange', 0.36))     # 安全距离，离障碍物过近舍弃路径
predict_time = float(rospy.get_param('~predictTime', 0.88)) # 预测时间
predict_time_nav = float(rospy.get_param('~predictTimeNav', 0.2))   # 预测领航者时间
obs_range_last = obs_range
predict_time_last = predict_time

dt = float(rospy.get_param('~dt', 0.08))    # 路径段时间间隔
dv = float(rospy.get_param('~dv', 0.2))    # 速度增量
dw = float(rospy.get_param('~dw', 0.5))     # 角速度增量

dh = float(rospy.get_param('~dh', 0.6))    # 末端角度考虑距离
do = float(rospy.get_param('~do', 1.0))    # 障碍物考虑范围
dh_last = dh

# 动态窗口范围
vrange = rospy.get_param('~v', ('-0.15 1.25')).split(' ')  # 速度范围
vrange = list(map(lambda x: float(x), vrange))
wrange = rospy.get_param('~w', ('-2.8 2.8')).split(' ') # 角速度范围
wrange = list(map(lambda x: float(x), wrange))

# DWA评价指标及因数
dist_factor = float(rospy.get_param('~distFactor', 2.0))        # 障碍物距离权重
heading_factor = float(rospy.get_param('~headingFactor', 1.0))  # 朝向领航者权重
velocity_factor = float(rospy.get_param('~velocityFactor', 1.2))    # 速度权重
w_factor_k1 = float(rospy.get_param('~wFactorK1', 0.1))             # 角速度振荡权重
w_factor_k2 = float(rospy.get_param('~wFactorK2', 0.1))             # 角速度大小权重
target_factor = float(rospy.get_param('~targetFactor', 6.0))    # 距领航者距离权重
switch_dist = float(rospy.get_param('~switchDist', 2.5*L0))     # 切换距离
...
class Robot:
    def __init__(self):
        ...
        print(t, r)
        self.laser_yaw = yaw
        self.laser_trans = np.matrix([
            [np.cos(yaw), -np.sin(yaw), t[0]],
            [np.sin(yaw),  np.cos(yaw), t[1]],
            [          0,            0,    1],
        ])
    ...
    def laser_callback(self, data):
        ...
        v = data.ranges[0]
        P_b = np.array([v*np.cos(angle), v*np.sin(angle), 1]).reshape(3, 1)
        P_a = self.laser_trans * P_b
        x, y = float(P_a[0]), float(P_a[1])
        last_angle = np.rad2deg(math.atan2(y, x))
        last_v = pow(x**2+y**2, 0.5)
        self.obstacle = []
        obs_flag = 0
        for i, v in enumerate(data.ranges):
            if v < 0.001 or v > obs_range:   # 滤除不需要的点
                continue
            angle = data.angle_increment * i + data.angle_min
            _angle = np.rad2deg(angle + self.laser_yaw)
            if abs(_angle) > 160:
                continue
            P_b = np.array([v*np.cos(angle), v*np.sin(angle), 1]).reshape(3, 1)
            P_a = self.laser_trans * P_b
            ...
            if angle < lidar_range[0] or angle > lidar_range[1]:    # 滤除角度范围外的点
                continue
            if abs(last_v - v) < 0.2 and abs(last_angle - angle) < 10:   # 若相邻两点距离、角度较近，认为是同一障碍物
                if obs_flag == 1:
    ...
    def localplan(self):
        ...
        global heading_factor, target_factor, obs_range, predict_time, dh, dist_factor, velocity_factor
        if dist > switch_dist + 0.2:
            ...
            target_factor = target_factor_last / 2
            heading_factor = heading_factor_last * 2
        ...
        if not ONLY_DWA:
            ...
            v_follower = min(vrange[-1], v_follower)
        ...
        if nav_v+0.2 > vmin and dist < L0 * 2:
            vmax = min(vmax, nav_v+0.4)
        ...
        P_b = nav_traj[-1]
        P_b[0] -= float(target[1])
        P_b[1] -= float(target[2])
        P_b[2] = 1
        P_b = P_b.reshape(3, 1)
        T_ba = np.matrix([
        ...
        if len(valid_traj) == 0:    # 若没有有效路径
            ...
            recover_flag = 1
            if time() - recover_start < recover_time[-1] + 0.1:
                recover_flag = 2
            recover_start = time()
...
class Traj:
    def __init__(self, v, w, obstacle, theta, x, y, nav_point, nav_v, nav_w, x2=None, y2=None):
        ...
        while _time <= predict_time:
            ...
            if _time * v <= do:
                for obs in obstacle:
                    obs_dist = obs.minDist(p)
                    if obs_dist < safe_range: # 距离障碍物太近，路径无效
                        self.is_valid = False
                        if _time == 0:
                            # recover_w朝向离障碍物远的方向
                            global recover_w
        ...
        if self.is_valid:
            ...
            if v > 0:
                ...
                self.heading = 2*math.pi - (abs(p[-1] - theta))
            ...
            self.target = 1 / (pow((nav_point[0]-p[0])**2+(nav_point[1]-p[1])**2, 0.5) + 0.01)
